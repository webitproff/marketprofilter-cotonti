...
if ($current_cat !== '') {
    if (cot_plugin_active('multicatmarket')) {
        $where_sql = "((param_category = ?) OR (JSON_CONTAINS(param_multicats, JSON_QUOTE(?))) OR param_category = '')";
        $bind = [$current_cat, $current_cat];
    } else {
        $where_sql = "(param_category = ? OR param_category = '')";
        $bind = [$current_cat];
    }
} else {
    $where_sql = "param_category = ''";
    $bind = [];
}
