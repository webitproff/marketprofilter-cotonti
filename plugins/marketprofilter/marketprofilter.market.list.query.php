...
if ($current_cat !== '') {
    $safe_cat = $db->quote($current_cat);
    if (cot_plugin_active('multicatmarket')) {
        // Параметр привязан к категории напрямую ИЛИ она есть в массиве param_multicats
        $cat_cond = "((param_category = $safe_cat) OR (JSON_CONTAINS(param_multicats, JSON_QUOTE($safe_cat))))";
        // + глобальные параметры (без категории)
        $cat_cond = "($cat_cond OR param_category = '')";
    } else {
        $cat_cond = "(param_category = $safe_cat OR param_category = '')";
    }
} else {
    $cat_cond = "param_category = ''";
}
