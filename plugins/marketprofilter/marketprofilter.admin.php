...
global $db, $db_x, $cfg, $L, $structure;
...
if ($tab === 'edit' && $param_id > 0) {
    ...
    $form_values = [
        ...
        'param_multicats'  => null,      // Multicatmarket: JSON или null
    ];
} else {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && in_array($action, ['add', 'edit'])) {
    ...
    // Multicatmarket: получаем отмеченные дочерние категории
    $multicats_selected = cot_import('param_multicats', 'P', 'ARR'); // массив кодов
    $data['param_multicats'] = !empty($multicats_selected)
        ? json_encode(array_values($multicats_selected))
        : null;
...
if ($tab === 'add' || $tab === 'edit') {
    ...
    // === Multicatmarket Integration: блок выбора дочерних категорий ===
    // === Multicatmarket Integration: выбор любых категорий через дерево ===
    if (cot_plugin_active('multicatmarket')) {
        // Готовим кэш code => id
        $code_to_id = [];
        $res = $db->query("SELECT structure_code, structure_id FROM {$db->structure} WHERE structure_area = 'market'");
        ...
        // Определяем, активен ли перевод категорий
        ...
        $saved_multicats = [];
        if (!empty($form_values['param_multicats'])) {
            $saved_multicats = json_decode($form_values['param_multicats'], true) ?: [];
        }

        $multicats_html = '<div class="mb-3">';
        $multicats_html .= '<label class="form-label">' . $L['marketprofilter_param_multicats'] . '</label>';
        $multicats_html .= '<div class="form-text mb-2">' . $L['marketprofilter_param_multicats_hint'] . '</div>';
        $multicats_html .= '<div style="max-height: 480px; overflow-y: auto; width: 100%;">';
        $multicats_html .= '<ul class="list-group list-group-flush">';
        $multicats_html .= marketprofilter_admin_render_category_tree('', $saved_multicats, $code_to_id, $blacklist);
        $multicats_html .= '</ul></div>';
        $multicats_html .= '</div>';

        $form_fields .= $multicats_html;
    }
    // === конец блока multicatmarket ===	
...
if ($tab === 'list') {
    ...
    foreach ($parameters as $param) {
        // Multicats: формируем строку с перечнем дочерних категорий
        $multicats_display = '—';
        if (cot_plugin_active('multicatmarket') && !empty($param['param_multicats'])) {
            $mcodes = json_decode($param['param_multicats'], true) ?: [];
            $names = [];
            foreach ($mcodes as $code) {
                $names[] = htmlspecialchars($structure['market'][$code]['title'] ?? $code);
            }
            $multicats_display = implode(', ', $names);
        }
        ...
        $t->assign([
            ...
            'PARAM_MULTICATS' => $multicats_display,
